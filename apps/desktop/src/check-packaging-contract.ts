...
const localIpcProtocolSource = readFileSync(join(appDir, "src", "local-ipc-protocol.ts"), "utf8");
const localIpcPathsSource = readFileSync(join(appDir, "src", "local-ipc-paths.ts"), "utf8");
const omarchySetupSource = readFileSync(join(appDir, "src", "omarchy-setup.ts"), "utf8");
const waybarHelperSource = readFileSync(join(appDir, "src", "waybar-helper.ts"), "utf8");
const appActionsCoreSource = readFileSync(join(appDir, "src", "app-actions-core.ts"), "utf8");
...
assert.match(controlCenterPreloadSource, /getOmarchySetup:[\s\S]*?openpets:omarchy-setup-snapshot/, "Control Center preload must expose only the normalized Omarchy setup snapshot.");
assert.match(controlCenterPreloadSource, /runOmarchySetupAction:[\s\S]*?openpets:omarchy-setup-action/, "Control Center preload must expose the narrow Omarchy setup action route.");
assert.match(windowsSource, /openpets:omarchy-setup-snapshot[\s\S]*?assertAllowedSender\(event, \["control-center"\]\)/, "Omarchy setup snapshots must be restricted to the Control Center.");
assert.match(windowsSource, /openpets:omarchy-setup-action[\s\S]*?action !== "install"[\s\S]*?action !== "repair"[\s\S]*?action !== "doctor"[\s\S]*?action !== "remove"/, "Omarchy setup actions must use an explicit fixed allowlist.");
assert.match(localIpcProtocolSource, /"status\.subscribe"/, "Local IPC must retain the authenticated event-driven Waybar status subscription.");
assert.match(omarchySetupSource, /"openpets-managed": true/, "Managed Waybar configuration must carry an explicit ownership marker.");
assert.match(omarchySetupSource, /walkerStart = "# OPENPETS:WALKER_MENU:START"/, "Managed Walker integration must retain its ownership marker.");
assert.match(omarchySetupSource, /name = "openpets"/, "Managed Walker integration must retain its dedicated provider name.");
assert.match(omarchySetupSource, /--toggle-default-pet-paused/, "Managed Walker integration must retain fixed desktop actions.");
assert.match(waybarHelperSource, /ELECTRON_RUN_AS_NODE=1|status\.subscribe/, "The Waybar helper must use the packaged runtime and authenticated status subscription.");
assert.doesNotMatch(waybarHelperSource, /execSync|spawnSync|shell:\s*true/, "The Waybar helper must not expose shell execution.");
assert.match(appActionsCoreSource, /--open-control-center[\s\S]*?--toggle-default-pet[\s\S]*?--toggle-default-pet-paused/, "Waybar desktop actions must remain a closed argument allowlist.");
